      ...
      <div className="bg-[#FAF7F2] border border-[#D8C7B5] rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden text-[#1F2937]">
        ...
        <div className="p-4 border-b border-[#E5D7C5] flex items-center gap-3 bg-[#F4EDE2]">
          ...
          <input
            ...
            placeholder="Buscar por receitas, ingredientes (ex: gengibre, sono, digestão)..."
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
          {!searchTerm.trim() ? (
            <div className="text-center py-10 text-[#374151]">
              ...
              <p className="font-serif text-lg font-bold">Digite algo para buscar no livro da Mestra Lin</p>
              <p className="text-sm text-[#4B5563] mt-1 font-serif">Exemplos: "Gengibre", "Pressão Alta", "Capítulo 4", "Mel"</p>
            </div>
          ) : (
            <>
              {/* Pages Results */}
              {searchResults.pages.length > 0 && (
                <div>
                  <h3 className="text-xs font-bold uppercase tracking-wider text-[#8C3A2B] mb-2 px-2 flex items-center gap-1.5">
                    <BookOpen className="w-4 h-4" /> Seções do Livro ({searchResults.pages.length})
                  </h3>
                      ...
                      return (
                        ...
                        >
                          <div>
                            ...
                            <span className="text-xs text-[#4B5563] font-mono">Página {p.pageNumber}</span>
                          </div>
              ...
              {searchResults.recipes.length > 0 ? (
                <div>
                  <h3 className="text-xs font-bold uppercase tracking-wider text-[#8C3A2B] mb-2 px-2 flex items-center gap-1.5">
                    <Utensils className="w-4 h-4" /> Receitas Encontradas ({searchResults.recipes.length})
                  </h3>
